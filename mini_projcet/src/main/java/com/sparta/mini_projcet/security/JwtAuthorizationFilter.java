...
////        // 토큰에서 유효시간 빼기
//        Date expireDate = JWT.require(JCEMapper.Algorithm.HMAC256(JwtProperties.secretKey)).build().verify(jwtToken).getClaim("expireDate").asDate();
...
//            Member userEntity = memberRepository.findByUsername(username).orElseThrow(()-> new IllegalArgumentException("유저가 존재하지 않습니다."));
//
